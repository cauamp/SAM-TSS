...
class rtmvss(nn.Module):
    def __init__(self, args, device):
        ...
        self.class_embed_cross_attn = cross_attention(h=8, d_model=256)
    ...
    def _forward_internal(self, imgs, depths, is_mem=True, is_training=True, current_ti=0):
        ...
        if is_training:
            ...
            for ti in range(0, frames):
                ...
                # Cross-attention: class queries attend to spatial embed
                class_queries = self.class_query.unsqueeze(0).expand(bsz, -1, -1)  # [bsz, num_classes, class_query_size]
                class_q = self.class_query_proj(class_queries)   # [bsz, num_classes, 256]
                embed_kv = self.embed_proj(embed)                # [bsz, num_queries, 256]

                sparse_embeddings = self.class_embed_cross_attn(
                    class_q.transpose(1, 2).contiguous(),        # [bsz, 256, num_classes]
                    embed_kv.transpose(1, 2).contiguous()        # [bsz, 256, num_queries]
                ).transpose(1, 2).contiguous()                   # [bsz, num_classes, 256]

                # Reshape for mask decoder: [bsz*num_classes, 1, 256]
                sparse_embeddings = sparse_embeddings.view(bsz * self.num_classes, 1, 256)
