...
class rtmvss(nn.Module):
    ...
    def _configure_multiclass_sam_mask_decoder(self):
        """Reconfigure SAM decoder mask tokens to emit one channel per semantic class."""
        ...
        decoder.num_mask_tokens = self.num_classes
        decoder.num_multimask_outputs = max(self.num_classes - 1, 0)

        new_mask_tokens = nn.Embedding(self.num_classes, transformer_dim).to(device=device, dtype=dtype)
        with torch.no_grad():
            copy_count = min(self.num_classes, old_mask_tokens.weight.shape[0])
            new_mask_tokens.weight[:copy_count].copy_(old_mask_tokens.weight[:copy_count])
            if self.num_classes > copy_count:
                new_mask_tokens.weight[copy_count:].copy_(
                    old_mask_tokens.weight[0].unsqueeze(0).expand(self.num_classes - copy_count, -1)
                )
        ...
        for i in range(self.num_classes):
            ...
            src_idx = i if i < len(old_hyper_mlps) else 0
        ...
        with torch.no_grad():
            ...
            copy_count = min(self.num_classes, old_last.out_features)
            ...
            if self.num_classes > copy_count:
                new_last.weight[copy_count:].copy_(
                    old_last.weight[0].unsqueeze(0).expand(self.num_classes - copy_count, -1)
                )
                new_last.bias[copy_count:].copy_(old_last.bias[0].expand(self.num_classes - copy_count))
    ...
    def _decode_all_class_masks(self, image_embeddings, dense_embeddings, sparse_embeddings, high_res_features):
        """Run the SAM mask decoder once and return all class masks as [B, C, H, W]."""
        feat_h, feat_w = image_embeddings.shape[-2:]

        low_res_multimasks, _, _, _ = self.sam2.sam_mask_decoder.predict_masks(
        ...
        if low_res_multimasks.shape[1] != self.num_classes:
            raise RuntimeError(f"Expected {self.num_classes} decoder channels, got {low_res_multimasks.shape[1]}")

        return low_res_multimasks.float().contiguous()
