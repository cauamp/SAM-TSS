...
def plot_training_log(run_path):
    ...
    ax2.plot(best_epoch, best_miou, marker='*', markersize=15, color='red', label=f'Best Val mIoU ({best_miou:.2f}% @ Epoch {int(best_epoch)})')
    ...
    ax3 = axes[2]
    ax3.plot(df['Epoch'], df['learningRate'], label='Learning Rate', marker='o', markersize=3, linewidth=1.5, color='green')
    ...
    if (df['Train-mIoU'] != 0).any():
        ...
        print(f"Final Train mIoU: Not tracked")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python plot_training_log.py <path_to_run_folder>")
        print("Example: python plot_training_log.py /Users/cauamp/Developer/jj/RTMVSS/runs/run_57964934")
        sys.exit(1)
