...
class rtmvss(nn.Module):
    def __init__(self, args, device):
        ...
        self.class_token_proj = nn.Linear(self.class_query_size, 256)
        self.query_token_proj = nn.Linear(256, 256)
    ...
    def decode_all_masks(self, image_features, query_embeddings):
        """Decode all class masks in one pass from dense features and query context."""
        mask_features = self.mask_feature_proj(image_features)
        pooled_query = query_embeddings.mean(dim=1)

        class_tokens = self.class_token_proj(self.class_query).unsqueeze(0)
        query_tokens = self.query_token_proj(pooled_query).unsqueeze(1)
        class_tokens = F.normalize(class_tokens + query_tokens, dim=-1)

        mask_features = F.normalize(mask_features, dim=1)
        return torch.einsum("bchw,bkc->bkhw", mask_features, class_tokens)
    ...
    def _forward_internal(self, imgs, depths, is_mem=True, is_training=True, current_ti=0):
        ...
        if is_training:
            ...
            for ti in range(0, frames):
                ...
                low_res_multimasks = self.decode_all_masks(feats_img[-1][:, ti], video_query_embeddings).float()
            ...
            low_res_multimasks = self.decode_all_masks(feats_img[-1][:, 0], video_query_embeddings).float()
            sam_high_masks_logits = F.interpolate(
                low_res_multimasks,
                size=(h, w),
                mode="bilinear",
            ...
            # print(f"Low-res multimasks shape (custom decoder output): {low_res_multimasks.shape} ")  # Debug print
