...
class rtmvss(nn.Module):
    def __init__(self, args, device):
        ...
        if args.enable_memory:
            ...
            self.enable_mem = False if (hasattr(args, 'baseline_mode') and args.baseline_mode) else True
    ...
    def forward(self, input, thermal, step=0, epoch=0, is_mem=None):
        """
        ...
        """
        if is_mem is None:
            is_mem = self.enable_mem  # Default to using memory during training, not during inference
    ...
    def _forward_internal(self, imgs, depths, is_mem=True, is_training=True, current_ti=0):
        if  self.video_query_initialized:
        ...
        if is_training:
            ...
            for ti in range(0, frames):
                ...
                print(f"[DEBUG] sparse_embeddings_expanded shape: {sparse_embeddings_expanded.shape}")

                Q = sparse_embeddings_expanded.size(1)

                print(f"[DEBUG] self.class_query shape: {self.class_query.shape}")
                ...
                print(f"[DEBUG] class_queries_expanded shape: {class_queries_expanded.shape}")
