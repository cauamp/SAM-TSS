...
class rtmvss(nn.Module):
    def __init__(self, args, device):
        ...
        use_video_memory = getattr(args, "enable_memory", False)
        aux_in_channels = self.num_video_queries if use_video_memory else self.num_frame_queries
        self.aux_sparse_head = nn.Sequential(
            nn.Conv2d(aux_in_channels, aux_in_channels, 3, padding=1, bias=True),
            nn.ReLU(),
            nn.Conv2d(aux_in_channels, 1, 1, bias=True),
        )
    ...
    def _forward_internal(self, imgs, depths, is_mem=True, is_training=True, current_ti=0):
        ...
        if is_training:
            ...
            for ti in range(0, frames):
                ...
                current_feat = feats_img[-1][:, ti]
                _, _, feat_h, feat_w = current_feat.shape
                ...
                if is_mem:
                    frame_query_feat = torch.einsum(
                        "bqc,bchw->bqhw", frame_query_embeddings, current_feat
                    ).contiguous()
                ...
                video_embed_with_current_feat = torch.einsum(
                    "bqc,bchw->bqhw", video_query_embeddings, current_feat
                ).contiguous()
                ...
                image_embeddings_expanded = current_feat.unsqueeze(1).expand(-1, self.num_classes, -1, -1, -1).reshape(bsz * self.num_classes, *current_feat.shape[1:]).contiguous()
                ...
                # Auxiliary prediction that bypasses the SAM mask decoder
                aux_sparse_feat = torch.einsum(
                    "bqc,bchw->bqhw", sparse_embeddings, image_embeddings_expanded
                ).contiguous()
                aux_sparse_logits_lowres = self.aux_sparse_head(aux_sparse_feat)
                aux_sparse_logits_lowres = aux_sparse_logits_lowres.view(bsz, self.num_classes, feat_h, feat_w)
                aux_sparse_logits = F.interpolate(
                    aux_sparse_logits_lowres, size=(h, w), mode="bilinear", align_corners=False
                )
                aux_sparse_preds.append(aux_sparse_logits)
                ...
                if is_mem:
                    ...
                    self.update_video_queries(current_feat, foreground_mask)
            ...
            if self.always_decode:
                aux_sparse_output = torch.stack(aux_sparse_preds, dim=1)
            else:
                aux_sparse_output = aux_sparse_preds[-1].unsqueeze(1)
            ...
            return main_pred_logits, aux_sparse_output, None, aux_fusion_logits, None
        else:
            if is_mem and not self.video_query_initialized:
                self.video_query_weight = self.video_queries.weight.unsqueeze(1).repeat(1, bsz, 1).contiguous()
                self.video_query_initialized = True
            ...
            aux_sparse_output = aux_sparse_logits.unsqueeze(1)
            
            # Return 5-tuple: (main, aux_rgb, aux_thermal, aux_fusion, features)
            return main_pred_logits, aux_sparse_output, None, None, None
