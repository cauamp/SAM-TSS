...
class rtmvss(nn.Module):
    def __init__(self, args, device):
        ...
        # Project spatial embeddings to 256-d
        self.embed_proj = nn.Linear(self._bb_feat_sizes[-1][0] * self._bb_feat_sizes[-1][1], 256)
        # Class-specific modulation - project each class query
        self.class_query_proj = nn.Linear(self.class_query_size, 256)
                
        # Each class has num_frame_queries learnable queries
        self.class_query = nn.Parameter(torch.empty(self.num_classes, self.num_frame_queries, self.class_query_size))
        ...
        if is_training:
            ...
            for ti in range(0, frames):
                ...
                if is_mem:
                    ...
                    num_queries = self.num_frame_queries

                # print(f"[DEBUG] embed shape: {embed.shape}")

                # Project spatial embeddings: [bsz, num_queries, spatial] -> [bsz, num_queries, 256]
                embed_proj = self.embed_proj(embed)  # [bsz, num_queries, 256]
                # print(f"[DEBUG] embed_proj shape: {embed_proj.shape}")
                
                # Expand spatial embeddings for each class: [bsz, num_queries, 256] -> [bsz, num_classes, num_queries, 256]
                sparse_embeddings = embed_proj.unsqueeze(1).expand(-1, self.num_classes, -1, -1)  # [bsz, num_classes, num_queries, 256]
                # print(f"[DEBUG] sparse_embeddings after expand shape: {sparse_embeddings.shape}")
                
                # Add class-specific modulation with per-query class embeddings
                # class_query: [num_classes, num_frame_queries, class_query_size]
                # Select first num_queries class queries to match spatial embeddings
                class_queries_full = self.class_query.unsqueeze(0).expand(bsz, -1, -1, -1)  # [bsz, num_classes, num_frame_queries, class_query_size]
                class_queries = class_queries_full[:, :, :num_queries, :]  # [bsz, num_classes, num_queries, class_query_size]
                # print(f"[DEBUG] class_queries shape: {class_queries.shape}")
                
                # Project each class query: reshape, project, reshape back
                class_queries_flat = class_queries.reshape(bsz * self.num_classes * num_queries, self.class_query_size)
                class_modulation_flat = self.class_query_proj(class_queries_flat)  # [bsz*num_classes*num_queries, 256]
                class_modulation = class_modulation_flat.reshape(bsz, self.num_classes, num_queries, 256)
                # print(f"[DEBUG] class_modulation shape: {class_modulation.shape}")
                
                # Add to spatial embeddings: [bsz, num_classes, num_queries, 256] + [bsz, num_classes, num_queries, 256]
                sparse_embeddings = sparse_embeddings + class_modulation
                # print(f"[DEBUG] sparse_embeddings after modulation shape: {sparse_embeddings.shape}")
                
                # Reshape for mask decoder: [bsz*num_classes, num_queries, 256]
                sparse_embeddings = sparse_embeddings.view(bsz * self.num_classes, num_queries, 256).contiguous()
                # print(f"[DEBUG] sparse_embeddings reshaped for mask decoder: {sparse_embeddings.shape}\n\n")
                ...
                if is_mem:
                    ...
                    # Pad foreground mask so that after mask_downsampler (total_stride=16) it matches pix_feat
                    _, _, mask_h, mask_w = foreground_mask.shape
                    target_h, target_w = feat_h * 16, feat_w * 16
                    pad_h = target_h - mask_h
                    pad_w = target_w - mask_w
                    if pad_h > 0 or pad_w > 0:
                        foreground_mask = F.pad(foreground_mask, (0, pad_w, 0, pad_h))
            ...
            # print(f"[DEBUG INFERENCE] embed shape: {embed.shape}")

            # Project spatial embeddings: [bsz, num_queries, spatial] -> [bsz, num_queries, 256]
            embed_proj = self.embed_proj(embed)  # [bsz, num_queries, 256]
            # print(f"[DEBUG INFERENCE] embed_proj shape: {embed_proj.shape}")
            
            # Expand spatial embeddings for each class: [bsz, num_queries, 256] -> [bsz, num_classes, num_queries, 256]
            sparse_embeddings = embed_proj.unsqueeze(1).expand(-1, self.num_classes, -1, -1)  # [bsz, num_classes, num_queries, 256]
            # print(f"[DEBUG INFERENCE] sparse_embeddings after expand shape: {sparse_embeddings.shape}")
            ...
            class_queries_full = self.class_query.unsqueeze(0).expand(bsz, -1, -1, -1)  # [bsz, num_classes, num_frame_queries, class_query_size]
            class_queries = class_queries_full[:, :, :num_queries, :]  # [bsz, num_classes, num_queries, class_query_size]
            # print(f"[DEBUG INFERENCE] class_queries shape: {class_queries.shape}")
            
            # Project each class query: reshape, project, reshape back
            class_queries_flat = class_queries.reshape(bsz * self.num_classes * num_queries, self.class_query_size)
            class_modulation_flat = self.class_query_proj(class_queries_flat)  # [bsz*num_classes*num_queries, 256]
            class_modulation = class_modulation_flat.reshape(bsz, self.num_classes, num_queries, 256)
            # print(f"[DEBUG INFERENCE] class_modulation shape: {class_modulation.shape}")
            
            # Add to spatial embeddings: [bsz, num_classes, num_queries, 256] + [bsz, num_classes, num_queries, 256]
            sparse_embeddings = sparse_embeddings + class_modulation
            # print(f"[DEBUG INFERENCE] sparse_embeddings after modulation shape: {sparse_embeddings.shape}")
            
            # Reshape for mask decoder: [bsz*num_classes, num_queries, 256]
            sparse_embeddings = sparse_embeddings.view(bsz * self.num_classes, num_queries, 256).contiguous()
            # print(f"[DEBUG INFERENCE] sparse_embeddings reshaped for mask decoder: {sparse_embeddings.shape}\n\n")
