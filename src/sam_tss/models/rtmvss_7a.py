...
class rtmvss(nn.Module):
    ...
    def _build_class_frame_queries(self, bsz):
        frame_query_weight = self.frame_queries.weight.view(
            self.num_classes, self.num_frame_queries, self.hidden_dim
        )
        frame_query_weight = frame_query_weight.unsqueeze(2).expand(-1, -1, bsz, -1).contiguous()
        frame_query_weight_flat = frame_query_weight.view(self.num_classes * self.num_frame_queries, bsz, self.hidden_dim)
        ...
        if is_training:
            ...
            for ti in range(0, frames):
                ...
                if is_mem:
                    ...
                    video_query_embeddings = self.video_queires_embed(
                        self.video_query_weight.view(self.num_classes * self.num_video_queries, bsz, self.hidden_dim)
                    ).view(bsz, self.num_classes, self.num_video_queries, -1).contiguous()
                    ...
                    video_query_embeddings = (
                        ...
                        .transpose(1, 2).view(bsz, self.num_classes, self.num_video_queries, -1)
                        .contiguous()
                    )
                else:
                ...
                # print(f"High-res mask shape (logits): {sam_high_mask.shape}")  # Debug print
            ...
            sparse_embeddings = self.sparse_embed(embed 
                    )# [B*C, Q, 256]
            ...
            # print(f"High-res mask shape (logits): {sam_high_mask.shape} \n\n")  # Debug print
            # Squeeze channel dimension: [bsz*num_classes, 1, h, w] -> [bsz*num_classes, h, w]
            sam_high_mask = sam_high_mask.squeeze(1)
            # Reshape back: [bsz*num_classes, h, w] -> [bsz, num_classes, h, w]
            sam_high_masks_logits = sam_high_mask.view(bsz, self.num_classes, h, w).contiguous()
    ...
    def update_video_queries(self, pix_feat, pred_masks_high_res, class_idx, alpha=0.1):
        ...
        class_selector = F.one_hot(
            torch.tensor(class_idx, device=updated_query.device),
            num_classes=self.num_classes,
        ).view(self.num_classes, 1, 1, 1).type_as(updated_query)
