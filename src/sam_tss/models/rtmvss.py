    ...
    def forward(self, imgs, depths, is_mem=True, is_training=True, current_ti=0):
        ...
        if is_training:
            ...
            for ti in range(0, frames):
                ...
                sam_high_masks = sam_high_mask.view(bsz, self.num_classes, h, w).contiguous()

                if is_mem:
                    # update video queries based on the predicted masks of all classes
                    # Reshape for update: [bsz, num_classes, num_queries, h, w] -> [bsz, num_classes*num_queries, h, w]
                    masks_for_update = sam_high_masks.view(bsz * self.num_classes, 1, h, w).contiguous()
            ...
            class_queries_expanded = class_queries.expand(-1, Q, -1)            # [B*C, Q, 256]
            print("class_queries_expanded shape:", class_queries_expanded.shape)
            print("sparse_embeddings_expanded shape:", sparse_embeddings_expanded.shape)
            # Fuse class query and sparse embedding
            sparse_embeddings_for_class = self.sparse_embed_for_class(
                torch.cat([class_queries_expanded, sparse_embeddings_expanded], dim=2)
            ) # [B*C, Q, 512]

            print("sparse_embeddings_for_class shape:", sparse_embeddings_for_class.shape)
            ...
            # Reshape back: [bsz*num_classes, h, w] -> [bsz, num_classes, h, w]
            sam_high_masks = sam_high_mask.view(bsz, self.num_classes, h, w).contiguous()
            print("sam_high_masks shape:", sam_high_masks.shape)
            
            if is_mem:
                # update video queries based on the predicted masks of all classes
                # Reshape for update: [bsz, num_classes, h, w] -> [bsz*num_classes, 1, h, w]
                masks_for_update = sam_high_masks.view(bsz * self.num_classes, 1, h, w).contiguous()
                print("masks_for_update shape:", masks_for_update.shape)
                self.update_video_queries(feats_img[-1][:, 0], masks_for_update)

            return sam_high_masks
